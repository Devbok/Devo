...
# --- Aapke Original Functions (Jerry Style) ---
...
async def get_thumb(videoid: str, user_id: int, user_name: str):
    # 1. CLEANUP & CACHE (Jerry style animation support)
    if not os.path.exists("cache"):
        os.mkdir("cache")
    else:
        # Har naye gaane par purana kachra saaf karna
    ...
    try:
        ...
        # 2. BACKGROUND & BLUR (Fixing Jerry's blur to 25)
        ...
        # 8. PROFESSIONAL ICONS (Transparency Added)
        if os.path.exists(icons_path):
            icons = Image.open(icons_path).convert("RGBA").resize((145, 30))
            r, g, b, a = icons.split(); a = a.point(lambda i: i * 0.8) # 80% Opacity
            icons = Image.merge('RGBA', (r, g, b, a))
            bg.paste(icons, (655, 415), icons)

        # 9. REQUESTED BY (Jerry style positioning)
